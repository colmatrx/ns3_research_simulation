...
int blacklistMaliciousVotingServers(int maliciousThreshold){	//recommended to be called on the werification queue after voting so malicious servers cannot be used for subsequent probing or working
								//it takes integer as argument and returns the number of blacklisted serverIDs
	set<string> allVerificationServerSet;	
	
	int numberOfMaliciousFlags, numberOfBlacklikstedServers = 0;		
...
void displayBlacklist(){	//function to display the blacklisted servers

	cout<<"Displaying Blacklist"<<endl;
	
	if (blacklistedServers.size() == 1)	{
	
		cout<<"["<<blacklistedServers[0]<<"]"<<endl;
	}
...
int main(){
	...
	myQueue.displayVerificationQueue();
	int mal = myQueue.blacklistMaliciousVotingServers(1);	 
