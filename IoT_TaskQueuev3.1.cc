...
TaskQueue(int numberOfServers){	//server naming convention is server1, server2.....servern
	...
	for (int i = 1; i <= numberOfServers; i++){
		...
		initializeProbeServer("server"+to_string(i));	//call initializeProbeServer to initialize the serverID - frequency pair in a vector with frequency of 0		
	}	
...
int initializeProbeServer(string serverID){	//it returns 1 if initialization is successful, it returns 0 if serverID already exists and -1 if it fails
...
int updateProbeServers(vector<string> serverIDs){	//takes a vector of serverIDs; returns 1 if successful and -1 if it fails
	...
	for (auto it = serverIDs.begin(); it != serverIDs.end(); it++){
		...
		if (serverIndex != ProbeServerMap.end()){	//true if serverID is found in map
			...
			initializeProbeServer(*it); //else part means it's a new server so call initializeProbeServer() to initialize it.
...
//overloaded updateProbeServers() to accept string

int updateProbeServers(string serverID){	//it returns 1 if updating is successful, it returns 0 if initializeProbeServer() was called and -1 if it fails
	...
	if (serverIndex != ProbeServerMap.end()){	//true if serverID is found in map
		...
		initializeProbeServer(serverID); //else part means it's a new server so call initializeProbeServer() to initialize it.
...
void displayProbeServer_Frequency(){	//to display the probing servers map data structure
	...
	cout<<"Probe Server Frequencies: node[frequency]"<<endl;
...
void displayProbeQueue(){	//prints out the content of the ProbeQueue
	...
	cout<<"Displaying Probe Queue"<<endl;	
...
void displayVerificationQueue(){	//prints out the content of the VerificationQueue
	...
	cout<<"Displaying Verification Queue"<<endl;	
	outputStream<<setw(10)<<left<<"TaskID"<<setw(20)<<left<<"Task_Timestamp"<<setw(20)<<left<<"Task_VotedResult"<<setw(10)<<left<<"serverID[serverResult, serverFlag]"<<endl;
	outputString = outputStream.str();
	cout<<outputString;
...
int voteOnProbeQueue(int numberOfVoters, int winThreshold){	//numberOfVoters = number of probe servers; winThreshold = minimum fraction of voters that should win the election eg 5 voters but need 4 correct results to win
	...
	int serverResultFrequency = 0, cumulativeServerResultFrequency = 0;	//to track the number of times a serverResult appears per TaskID
	...
	for (auto it = ProbeQueue.begin(); it != ProbeQueue.end(); it++){	//loop to traverse the map
		...
		for (auto vec_itr = it->second.begin(); vec_itr != it->second.end(); vec_itr++){	//loop to traverse the vector of each TaskID in the map
			...
			if (vec_itr->server.serverResult == ""){	//if a server still has the default invalid server result
	...
	if (it != VerificationQueue.end()){	//true if server's taskID is found
		...
		for(int i = 0; i < it->second.size(); i++){	//double check the serverID is not present in the vector before adding it
			...
			if(it->second[i].server.serverID == serverID){	//true if server has seen this task before
				...
				cout<<"error: in updateVerificationQueue; serverID already saw this TaskID before; this is not a valid update action"<<endl;
				return -1;
			}
		}
...
int main(){
	...
	myNodes.push_back("node1"); myNodes.push_back("node2"); myNodes.push_back("node3"); myNodes.push_back("node4");
	TaskQueue myQueue(myNodes);		//create object of class TaskQueue
	myQueue.displayProbeServer_Frequency();
	myQueue.updateProbeServers(myNodes);
	myQueue.displayProbeServer_Frequency();
	myQueue.updateProbeServers("node5");
	myQueue.displayProbeServer_Frequency();
	myQueue.updateProbeServers("node6");
	myQueue.displayProbeServer_Frequency();
